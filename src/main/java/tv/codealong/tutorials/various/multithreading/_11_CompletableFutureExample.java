...
/**
 * CompletableFuture - механизм, в котором не нужно в ручную создавать ThreadPool.
 * CompletableFuture - Future на стероидах.)))
 * <p>
 * оздание потоков управляемым способом, когда мы сами контролируем сколько будет создаваться потоков и
...
public class _11_CompletableFutureExample {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        // Создавать пул потоков уже не нужно, т.к. мы будем использовать CompletableFuture.
        // А CompletableFuture выполняет задачи в common пуле, который по умолчанию создается JVM один раз на весь наш
        // Java процесс. Этот пул нам не нужно создавать его самостоятельно, стартовать, останавливать.
        ...
        for (int i = 0; i < JUMPS; i++) {
            ...
            //Аналог старого варианта, как на Future:
...
//            }
            ...
            //Альтернативный вариант (поинтересней): сделаем композицию их них
            //В результате получаем один объект типа CompletableFuture.
            //В итоге, когда мы дожидаемся этого одного объекта, мы дожидаемся выполнения всех задач.
            CompletableFuture<?> futures = CompletableFuture.allOf(
                    Stream.generate(() -> CompletableFuture.runAsync(runnable))
                            .limit(THREADS)
