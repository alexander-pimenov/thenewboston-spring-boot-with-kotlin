...
/**
 * 🔹 Callback (Колбэк) — "Перезвони мне, когда будет готово!"
 * Это старый, но простой подход для асинхронных операций.
 * 🔸 Как работает?
 * Ты передаёшь функцию (callback), которая выполнится после завершения задачи.
 ...
 * ✔ Подходит для событийных моделей (например, клик кнопки в UI)
 * <p>
 * 🔸 Минусы колбэков:
 * ❌ Callback Hell — если нужно сделать несколько асинхронных операций подряд, код превращается в лестницу из })})}),
 * т.е. много вложенных колбэков, код становится нечитаемым
 * ❌ Сложно обрабатывать ошибки
 * ❌ Нет удобной композиции операций
...
//        });
    ...
    //
    //// Использование
    //fetchData { result -> println(result) } // Выведет "Данные получены!" через 1 сек
}
